...
function ChatContent() {
  ...
  function linkifyCitations(html: string, sourcesCount: number): string {
    ...
    return html.replace(/\b(Sources?)\s*#?\s*(\d+(?:\s*(?:,|and|&)\s*\d+)*)\b/gi, (match, _label, numList) => {
      const nums = Array.from(numList.matchAll(/\d+/g)).map((m) => parseInt((m as RegExpMatchArray)[0], 10));
    ...
    <>
      <style>{`
        ...
        .pp-bubble-ai {
          ...
          box-shadow: 0 4px 24px rgba(0,0,0,0.4); word-break: break-word; overflow: hidden;
        }
        .pp-bubble-ai::before {
