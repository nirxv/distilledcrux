...
.pyq-year{font-family:var(--font-mono);font-size:0.62rem; font-weight: 500;padding:2px 9px;border-radius:3px;background:rgba(67,97,238,0.08);border:1px solid rgba(67,97,238,0.18);color:var(--accent);}
...
.m60{background:rgba(167,139,250,0.06);border:1px solid rgba(167,139,250,0.15);color:#a78bfa;}
...
.pyq-attempt{padding:5px 14px;border-radius:4px;border:1px solid rgba(67,97,238,0.2);background:rgba(67,97,238,0.05);color:var(--accent);font-family:var(--font-mono);font-size:0.62rem; font-weight: 500;letter-spacing:0.06em;cursor:pointer;transition:all 0.15s;}
.pyq-attempt:hover{background:rgba(67,97,238,0.1);border-color:rgba(67,97,238,0.35);}
...
export default function PubAdminPYQsPage() {
  ...
  const handleAttempt = (e: React.MouseEvent, q: any) => {
    e.stopPropagation();
    const p = new URLSearchParams({ subject: 'pub-admin', question: q.question, marks: String(q.marks || 10), year: q.year });
    router.push(`/evaluate?${p.toString()}`);
  };

  const mClass = (m: number | null) => {
    if (m === 10) return 'm10';
    if (m === 15) return 'm15';
    if (m === 20) return 'm20';
    if (m === 60) return 'm60';
        ...
        <div className="pyq-inner">
          ...
          <div className="pyq-head">
            <div className="pyq-head-top">
              <h1 className="pyq-title">Public Administration PYQs</h1>
          ...
          {filtered.length === 0 ? (
            ...
            <div className="pyq-cards">
              {filtered.map((q: any) => (
                <div key={q.id} className="pyq-card" onClick={() => router.push(`/pub-admin/pyqs/${q.id}`)}>
                  ...
                  <div className="pyq-card-bottom">
                    ...
                    <div style={{ display: 'flex', gap: '6px' }}>
                      ...
                      <button className="pyq-view" onClick={e => { e.stopPropagation(); router.push(`/pub-admin/pyqs/${q.id}`); }}>View →</button>
                    </div>
                  </div>
                </div>
              ))}
