...
const CSS = `
  ...
  .pd-badge.accent { background:rgba(248,113,113,0.08); border-color:rgba(248,113,113,0.2); color:#f87171; }
  ...
  .pd-model-card::before {
    ...
    background:linear-gradient(90deg,#f87171 0%,rgba(248,113,113,0.2) 60%,transparent 100%);
  }
  ...
  .pd-model-prose .bullet::before { content:'·'; color:#f87171; flex-shrink:0; font-size:1rem; line-height:1.6; }
  ...
  .pd-dot { width:6px; height:6px; border-radius:50%; background:#f87171;
...
export default function PSIRPYQDetailPage() {
  ...
  useEffect(() => {
    if (!pyq) return;
    fetch(`/api/pyq-answers?pyq_id=${pyq.id}&subject=polsci`)
      .then(r => r.json())
  ...
  const generateModelAnswer = async () => {
    ...
    try {
      const res = await fetch('/api/model-answer', {
        ...
        body: JSON.stringify({ question: pyq.question, marks: pyq.marks, subject: 'polsci', topic: pyq.topic }),
      });
  ...
  if (!pyq) return (
    <div className="pd-wrap">
      ...
      <div className="pd-empty">
        Question not found.{' '}
        <Link href="/polsci/pyqs" style={{ color: '#f87171' }}>← Back to PYQs</Link>
      </div>
    </div>
  );
        ...
        <div className="pd-inner">
          ...
          <div>
            ...
            <div className="pd-crumb">
              ...
              <Link href="/polsci/pyqs">PSIR PYQs</Link>
              <svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M4.5 3l3 3-3 3" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/></svg>
            ...
            <div className="pd-actions">
              <Link
                href={`/chat?subject=polsci&q=${encodeURIComponent(pyq.question)}`}
              ...
              <Link
                href={`/evaluate?question=${encodeURIComponent(pyq.question)}&marks=${pyq.marks}&subject=polsci`}
            ...
            {(generating || generated || modelAnswer) && (
              <div className="pd-model-card" style={{ marginBottom: '1.5rem' }}>
                <div className="pd-section-label">Model Answer · {pyq.marks} marks · PSIR Optional</div>
            ...
            <div className="pd-nav">
              {prevId ? (
                <Link href={`/polsci/pyqs/${prevId}`} className="pd-nav-btn">
              ...
              <Link href="/polsci/pyqs" className="pd-nav-btn">All PYQs</Link>
              {nextId ? (
                <Link href={`/polsci/pyqs/${nextId}`} className="pd-nav-btn">
                  Next
          ...
          <div className="pd-sidebar">
            ...
            <div className="pd-sidebar-card">
              ...
              <Link
                href={`/chat?subject=polsci&q=${encodeURIComponent(pyq.question)}&topic=${encodeURIComponent(pyq.topic)}`}
                className="pd-btn-primary"
            ...
            {related.length > 0 && (
              <div className="pd-sidebar-card">
                ...
                <div className="pd-related">
                  {related.map(q => (
                    <Link key={q.id} href={`/polsci/pyqs/${q.id}`} className="pd-related-item">
