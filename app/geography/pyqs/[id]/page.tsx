...
type PYQ = {
  id: number; year: string; paper: string; section: string;
  question: string; marks: number; topic: string;
};
type AnswerEntry = {
  id: string; display_name: string; public_url: string;
  answer_number: number; created_at: string;
};
...
.pd-badge{font-family:var(--font-mono);font-size:0.6rem;padding:2px 9px;border-radius:3px;border:1px solid var(--border);background:var(--bg3);color:var(--text3);}
.pd-badge.geo{background:var(--geo-dim);border-color:var(--geo-border);color:var(--geo);}
...
.pd-btn-primary{
  ...
  background:var(--geo);color:#000;
  padding:8px 18px;border-radius:6px;border:none;cursor:pointer;transition:opacity 0.15s;text-decoration:none;
}
.pd-btn-primary:hover{opacity:0.85;}
[data-theme="light"] .pd-btn-primary{color:#fff;}
.pd-btn-ghost{
  display:inline-flex;align-items:center;gap:6px;
  font-family:var(--font-ui);font-size:0.82rem;font-weight:500;
  background:transparent;color:var(--text2);
  padding:8px 18px;border-radius:6px;border:1px solid var(--border);cursor:pointer;transition:all 0.15s;text-decoration:none;
}
.pd-btn-ghost:hover{border-color:var(--geo-border);color:var(--geo);background:var(--geo-dim);}

.pd-section-label{font-family:var(--font-mono);font-size:0.58rem;letter-spacing:0.18em;text-transform:uppercase;color:var(--text3);margin-bottom:1rem;}
...
.pd-model-card::before{
  ...
  background:linear-gradient(90deg,var(--geo) 0%,var(--geo-border) 60%,transparent 100%);
}
...
.pd-model-prose .bullet::before{content:'·';color:var(--geo);flex-shrink:0;font-size:1rem;line-height:1.6;}
.pd-model-blur{filter:blur(5px);user-select:none;pointer-events:none;}
.pd-paywall{
  position:absolute;inset:0;display:flex;flex-direction:column;
  align-items:center;justify-content:center;gap:0.75rem;
  background:linear-gradient(to bottom,rgba(5,5,8,0) 0%,var(--bg) 40%);padding:2rem;
}
...
.pd-generating{display:flex;align-items:center;gap:8px;padding:1.5rem 0;font-family:var(--font-mono);font-size:0.72rem;color:var(--text3);letter-spacing:0.06em;}
@keyframes pd-dot{0%,80%,100%{opacity:0.2;transform:scale(0.7)}40%{opacity:1;transform:scale(1)}}
.pd-dot{width:6px;height:6px;border-radius:50%;background:var(--geo);animation:pd-dot 1.2s ease-in-out infinite;}
...
.pd-nav-btn:hover{border-color:var(--geo-border);color:var(--geo);background:var(--geo-dim);}

.pd-upload-card{background:var(--bg2);border:1px solid var(--border);border-radius:12px;padding:1.5rem;margin-bottom:1.5rem;}
.pd-upload-input{
  width:100%;background:var(--bg3);border:1px solid var(--border);
  border-radius:6px;padding:8px 12px;color:var(--text);
  font-family:var(--font-ui);font-size:0.84rem;outline:none;box-sizing:border-box;margin-bottom:0.6rem;
}
.pd-upload-input:focus{border-color:var(--geo-border2);}
.pd-answer-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:10px;margin-top:0.75rem;}
.pd-answer-card{background:var(--bg2);border:1px solid var(--border);border-radius:8px;padding:1rem 1.1rem;text-decoration:none;display:block;transition:background 0.15s;}
.pd-answer-card:hover{background:var(--bg3);border-color:var(--geo-border);}
.pd-answer-icon{font-size:1.3rem;margin-bottom:0.4rem;}
.pd-answer-name{font-family:var(--font-ui);font-size:0.82rem;color:var(--text);font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-bottom:2px;}
...
.pd-sidebar-card{background:var(--bg2);border:1px solid var(--border);border-radius:12px;padding:1.25rem;}
...
.pd-related-item{
  ...
  font-family:var(--font-ui);font-size:0.78rem;color:var(--text2);line-height:1.5;transition:all 0.15s;
}
.pd-related-item:hover{border-color:var(--geo-border);background:var(--geo-dim);color:var(--geo);}
...
export default function PYQDetailPage() {
  ...
  useEffect(() => {
    if (!pyq) return;
    fetch(`/api/pyq-answers?pyq_id=${pyq.id}&subject=geography`)
      .then(r => r.json())
  ...
  const generateModelAnswer = async () => {
    ...
    setGenerating(true); setGenerated(false); setModelAnswer('');
    ...
    try {
      const res = await fetch('/api/model-answer', {
        ...
        body: JSON.stringify({ question: pyq.question, marks: pyq.marks, subject: 'geography', topic: pyq.topic }),
      });
      if (!res.ok) { setModelAnswer('Failed to generate. Try again.'); setGenerating(false); return; }
      ...
      setGenerated(true);
    } catch { setModelAnswer('Something went wrong.'); }
  ...
  const handleUpload = async () => {
    ...
    if (!res.ok || data.error) {
      ...
      setAnswers((prev: any) => [data.answer, ...prev]);
      setFile(null); setDisplayName('');
      if (fileRef.current) fileRef.current.value = '';
      setUploadOk(true); setTimeout(() => setUploadOk(false), 3500);
    }
    setUploading(false);
  };

  const related = (questions as PYQ[]).filter(q => q.topic === pyq?.topic && q.id !== pyq?.id).slice(0, 5);
  ...
  if (!pyq) return (
    <div className="pd-wrap">
      ...
      <div className="pd-empty">Question not found. <Link href="/geography/pyqs" style={{ color: 'var(--geo)' }}>← Back</Link></div>
    </div>
  );
        ...
        <div className="pd-inner">
          ...
          <div>
            <div className="pd-crumb">
              <Link href="/">Home</Link>
              <Chevron dir="r" />
              <Link href="/geography/pyqs">Geography PYQs</Link>
              <Chevron dir="r" />
              <span style={{ color: 'var(--geo)' }}>{pyq.topic}</span>
            </div>

            <div className="pd-qcard">
              <div className="pd-badges">
                <span className="pd-badge geo">{pyq.year}</span>
                <span className="pd-badge geo">{pyq.paper}</span>
                {pyq.section && <span className="pd-badge">{pyq.section}</span>}
                <span className={`pd-badge ${marksClass(pyq.marks)}`}>{pyq.marks}M</span>
                <span className="pd-badge" style={{ color: 'var(--geo)', borderColor: 'var(--geo-border)', background: 'var(--geo-dim)' }}>{pyq.topic}</span>
              </div>
              <p className="pd-question">{pyq.question}</p>
            </div>

            <div className="pd-actions">
              <Link href={`/chat?subject=geography&q=${encodeURIComponent(pyq.question)}`} className="pd-btn-primary">
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 0 2 2z"/></svg>
                Ask AI
              </Link>
            ...
            {(generating || generated || modelAnswer) && (
              <div className="pd-model-card" style={{ marginBottom: '1.5rem' }}>
                <div className="pd-section-label">Model Answer · {pyq.marks} marks · Geography Optional</div>
                {generating && !modelAnswer && (
                  <div className="pd-generating">
                    <div className="pd-dot"/><div className="pd-dot"/><div className="pd-dot"/> Generating…
                  </div>
                )}
                {modelAnswer && (
                  <div style={{ position: 'relative' }}>
                    <div
                      ...
                      dangerouslySetInnerHTML={{ __html: formatModelAnswer(!isPremium && generated ? modelAnswer.slice(0, 600) + '…' : modelAnswer) }}
                    />
                    {!isPremium && generated && (
                      <div className="pd-paywall">
                        ...
                        <p className="pd-paywall-text">Full model answer on Premium</p>
                        <Link href="/pricing" className="pd-btn-primary" style={{ fontSize: '0.8rem', padding: '7px 16px' }}>Upgrade →</Link>
                      </div>
                    )}
                  </div>
                )}
            ...
            <div className="pd-upload-card">
              ...
              <input className="pd-upload-input" placeholder="Your name" value={displayName} onChange={e => setDisplayName(e.target.value)} />
              <input ref={fileRef} type="file" accept="application/pdf" onChange={e => setFile(e.target.files?.[0] ?? null)} style={{ color: 'var(--text2)', fontSize: '0.82rem', marginBottom: '0.6rem', display: 'block' }} />
              {uploadErr && <div style={{ color: 'var(--red)', fontSize: '0.8rem', marginBottom: '0.5rem' }}>{uploadErr}</div>}
              {uploadOk && <div style={{ color: 'var(--geo)', fontSize: '0.8rem', marginBottom: '0.5rem' }}>✓ Submitted!</div>}
              <button className="pd-btn-primary" onClick={handleUpload} disabled={uploading} style={{ opacity: uploading ? 0.6 : 1 }}>
                {uploading ? 'Uploading…' : 'Submit PDF'}
              </button>
            </div>

            <div>
              <div className="pd-section-label">Community Answers{answers.length > 0 ? ` · ${answers.length}` : ''}</div>
              {loadingAnswers ? (
                <div className="pd-generating"><div className="pd-dot"/><div className="pd-dot"/><div className="pd-dot"/></div>
              ) : answers.length === 0 ? (
                <div className="pd-dashed">No answers yet — be the first to submit.</div>
              ) : (
                <div className="pd-answer-grid">
                  {answers.map((ans: AnswerEntry) => (
                    <a key={ans.id} href={ans.public_url} target="_blank" rel="noopener noreferrer" className="pd-answer-card">
                      ...
                      <div className="pd-answer-meta">#{ans.answer_number} · {new Date(ans.created_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</div>
                    </a>
                  ))}
                </div>
              )}
            ...
            <div className="pd-nav">
              {prevId ? <Link href={`/geography/pyqs/${prevId}`} className="pd-nav-btn"><Chevron dir="l"/>Prev</Link> : <span/>}
              <Link href="/geography/pyqs" className="pd-nav-btn">All PYQs</Link>
              {nextId ? <Link href={`/geography/pyqs/${nextId}`} className="pd-nav-btn">Next<Chevron dir="r"/></Link> : <span/>}
            </div>
          </div>
          ...
          <div className="pd-sidebar">
            <div className="pd-sidebar-card">
              ...
              <p style={{ fontFamily: 'var(--font-ui)', fontSize: '0.8rem', color: 'var(--text3)', lineHeight: 1.6, marginBottom: '0.85rem' }}>
                Ask the AI to explain this question, suggest an outline, cite relevant concepts, or critique your draft.
              </p>
              <Link href={`/chat?subject=geography&q=${encodeURIComponent(pyq.question)}&topic=${encodeURIComponent(pyq.topic)}`} className="pd-btn-primary" style={{ width: '100%', justifyContent: 'center', boxSizing: 'border-box' }}>
                Open in AI Chat →
              </Link>
            </div>
            ...
            <div className="pd-sidebar-card">
              ...
              <div style={{ border: '1px dashed var(--border)', borderRadius: 8, padding: '1.25rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem', textAlign: 'center' }}>
                <span style={{ fontSize: '1.4rem' }}>🏆</span>
                <div style={{ fontFamily: 'var(--font-body)', fontSize: '0.85rem', fontWeight: 700, color: 'var(--text)' }}>Coming Soon</div>
                <div style={{ fontFamily: 'var(--font-ui)', fontSize: '0.75rem', color: 'var(--text3)', lineHeight: 1.6 }}>Topper copies for Geography will be added here.</div>
              </div>
            </div>
            ...
            {related.length > 0 && (
              <div className="pd-sidebar-card">
                ...
                <div className="pd-related">
                  {related.map(q => (
                    <Link key={q.id} href={`/geography/pyqs/${q.id}`} className="pd-related-item">
            ...
            <div className="pd-sidebar-card">
              ...
              {([['Year', pyq.year], ['Paper', pyq.paper], ['Marks', `${pyq.marks}M`], ['Topic', pyq.topic], pyq.section ? ['Section', pyq.section] : null] as (string[] | null)[])
