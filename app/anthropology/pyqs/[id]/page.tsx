...
export default function PYQDetailPage() {
  ...
  useEffect(() => {
    if (!pyq) return;
    fetch(`/api/pyq-answers?pyq_id=${pyq.id}&subject=anthropology`)
      .then(r => r.json())
  ...
  const generateModelAnswer = async () => {
    ...
    try {
      const res = await fetch('/api/model-answer', {
        ...
        body: JSON.stringify({ question: pyq.question, marks: pyq.marks, subject: 'anthropology', topic: pyq.topic }),
      });
  ...
  if (!pyq) return (
    <div className="pd-wrap">
      ...
      <div className="pd-empty">
        Question not found.{' '}
        <Link href="/anthropology/pyqs" style={{ color: 'var(--accent)' }}>← Back to PYQs</Link>
      </div>
    </div>
  );
        ...
        <div className="pd-inner">
          ...
          <div>
            ...
            <div className="pd-crumb">
              ...
              <Link href="/anthropology/pyqs">Anthropology PYQs</Link>
              <svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M4.5 3l3 3-3 3" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/></svg>
            ...
            <div className="pd-qcard">
              <div className="pd-badges">
                ...
                {(pyq as any).microtheme && <span className="pd-badge" style={{background:'rgba(168,85,247,0.07)',borderColor:'rgba(168,85,247,0.2)',color:'#c084fc'}}>{(pyq as any).microtheme}</span>}
              </div>
            ...
            <div className="pd-actions">
              <Link
                href={`/chat?subject=anthropology&q=${encodeURIComponent(pyq.question)}`}
            ...
            {(generating || generated || modelAnswer) && (
              <div className="pd-model-card" style={{ marginBottom: '1.5rem' }}>
                <div className="pd-section-label">Model Answer · {pyq.marks} marks · Anthropology Optional</div>
            ...
            <div className="pd-nav">
              {prevId ? (
                <Link href={`/anthropology/pyqs/${prevId}`} className="pd-nav-btn">
              ...
              <Link href="/anthropology/pyqs" className="pd-nav-btn">All PYQs</Link>
              {nextId ? (
                <Link href={`/anthropology/pyqs/${nextId}`} className="pd-nav-btn">
                  Next
          ...
          <div className="pd-sidebar">
            ...
            <div className="pd-sidebar-card">
              ...
              <Link
                href={`/chat?subject=anthropology&q=${encodeURIComponent(pyq.question)}&topic=${encodeURIComponent(pyq.topic)}`}
                className="pd-btn-primary"
            ...
            {related.length > 0 && (
              <div className="pd-sidebar-card">
                ...
                <div className="pd-related">
                  {related.map(q => (
                    <Link key={q.id} href={`/anthropology/pyqs/${q.id}`} className="pd-related-item">
